...
- カード内の裸の `https://` URLをリンク表示し、クリックで外部ブラウザの新しいタブに開く
  (表示のみ・ファイルは書き換えない。`http:` や `[label](url)` 形式は対象外。詳細は仕様書5.7)
...
1. **作成** — コマンドパレット →「Kanban Flow ボードを新規作成」。5レーンのボードが開く。
2. **無損失** — ファイルメニューの「Markdown として開く」でテンプレートを確認し、
   「Kanban Flow ボードとして開く」で戻す。
3. **追加 / 編集** —「+ カードを追加」で `➕ <今日>` が付与される。カードをダブルクリックで編集
...
5. **削除** — カードにホバーすると出るゴミ箱アイコンから。確認ダイアログが出る。
6. **アーカイブ** — 完了レーン(**Complete** マーカー付き = DONE)ヘッダーの ⋯ メニュー、または
   コマンドパレットの「完了カードを年月別にアーカイブ」。件数付きの確認ダイアログのあと、
   `## Archive` に `### YYYY-MM` 別で退避される。カード単位のアーカイブは無い。
7. **外部リンク** — カード内の裸の `https://` URL がリンク表示され、クリックで外部ブラウザに
   1回だけ開くこと。`http:` / `javascript:` / `data:` / `obsidian://` やユーザー情報付き URL
   (`https://example.com@evil.example/`)がリンクにならないこと。
8. **互換性** — 本プラグインで編集したボードを本家 obsidian-kanban で開く。逆に、タグ /
   `^ブロックID` / `@{}` 入りのファイルを本プラグインで開き、記法が壊れないことを確認。
9. **外部編集** — 別ペインや別端末でファイルを編集し、保存ループを起こさず再パース・再描画される
   ことを確認。
